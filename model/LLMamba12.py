...
class LLFormer(nn.Module):
    def __init__(self,
        ...
        #不同的kernel size需要weights进行结合
        self.coefficient_emb = nn.Parameter(torch.Tensor(np.ones((2, int(int(dim))))), requires_grad=attention)
        self.skip_emb = nn.Conv2d(int(int(dim)), int(int(dim)), kernel_size=1, bias=bias)
    ...
    def forward(self, inp_img):
        ...
        out_fusion_1_6 = self.coefficient_emb[0, :][None, :, None, None] * out_fusion_123 + self.coefficient_emb[1, :][None, :, None, None] * out_fusion_456
        out_fusion_1_6 = self.skip_emb(out_fusion_1_6)
        ...
        inp_fusion = Permute_layer1(torch.cat([out_1.unsqueeze(1),out_2.unsqueeze(1),out_3.unsqueeze(1)],dim=4))
        out_fusion_123 = self.layer_fussion_3(inp_fusion)
        out = self.conv_fuss_3(out_fusion_123)
