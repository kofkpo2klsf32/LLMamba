...
class Permute(nn.Module):
    ...
    #如果self.args是(0,2,1) 那就是张量的第二维和第三维交换
    def forward(self, x: torch.Tensor):
        return x.permute(*self.args)

class OverlapPatchEmbed(nn.Module):
    def __init__(self, in_c=3, embed_dim=48, patch_size=4, bias=False, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False):
        super(OverlapPatchEmbed, self).__init__()

        self.proj = nn.Conv2d(in_c, embed_dim, kernel_size=patch_size, stride=patch_size, bias=bias)
        self.permute = nn.Identity() if channel_first else Permute(0, 2, 3, 1)
...
class Downsample(nn.Module):
    def __init__(self, n_feat):
        ...
        #self.body = nn.Sequential(nn.Conv2d(n_feat, n_feat//2, kernel_size=3, stride=1, padding=1, bias=False),
                                  #nn.PixelUnshuffle(2))
        self.body = nn.Conv2d(n_feat, n_feat*2, kernel_size=2, stride=2, bias=False)
...
#resize to 128x128 before U-Net
class Resize(nn.Module):
    def __init__(self, in_dim, scale=8):
        super(Resize, self).__init__()

        self.body = nn.Sequential(nn.Conv2d(in_dim, in_dim*scale*scale, kernel_size=3, stride=1, padding=1, bias=False),
...
class LAM_Module_v2(nn.Module):  
    ...
    def forward(self,x):
        ...
        x_input = x.reshape(m_batchsize,N*C, height, width)
        qkv = self.qkv_dwconv(self.qkv(x_input))
        q, k, v = qkv.chunk(3, dim=1)
        #q = q.view(m_batchsize, N, -1)
        #k = k.view(m_batchsize, N, -1)
        #v = v.view(m_batchsize, N, -1)
        q = q.reshape(m_batchsize, N, -1)
        k = k.reshape(m_batchsize, N, -1)
        v = v.reshape(m_batchsize, N, -1)
...
class LLFormer(nn.Module):
    def __init__(self,
        ...
        self.patch_embed1 = OverlapPatchEmbed(inp_channels, dim, patch_size=8)
        self.patch_embed2 = OverlapPatchEmbed(inp_channels, dim, patch_size=2)
        self.patch_embed3 = OverlapPatchEmbed(dim, dim, patch_size=8)
        self.patch_embed4 = OverlapPatchEmbed(dim, dim, patch_size=2)
        ...
        self.encoder_1 = nn.Sequential(*[VSSM(dim=dim) for i in range(num_blocks[0])])
        ...
        self.encoder_2 = nn.Sequential(*[VSSM(dim=int(dim)) for i in range(num_blocks[0])])
        ...
        self.encoder_3 = nn.Sequential(*[VSSM(dim=int(dim)) for i in range(num_blocks[0])])
        
        self.encoder_4 = nn.Sequential(*[VSSM(dim=dim) for i in range(num_blocks[0])])

        self.encoder_5 = nn.Sequential(*[VSSM(dim=int(dim)) for i in range(num_blocks[0])])

        self.encoder_6 = nn.Sequential(*[VSSM(dim=int(dim)) for i in range(num_blocks[0])])

        self.layer_fussion_1 = LAM_Module_v2(in_dim=int(dim*3))
        self.conv_fuss_1 = nn.Conv2d(int(dim * 3), int(dim), kernel_size=1, bias=bias)

        self.layer_fussion_2 = LAM_Module_v2(in_dim=int(dim*3))
        self.conv_fuss_2 = nn.Conv2d(int(dim * 3), int(dim), kernel_size=1, bias=bias)

        self.resize1 = Resize(in_dim=dim, scale=4)
        #self.resize2 = Resize(in_dim=dim, scale=2)
        self.resize3 = Resize(in_dim=dim, scale=16)
        self.resize4 = Resize(in_dim=dim, scale=4)
        ...
        self.latent = nn.Sequential(*[VSSM(dim=int(dim)) for i in range(num_blocks[0])])
        ...
        self.decoder_level1_0 = nn.Sequential(*[VSSM(dim=int(int(dim * 2))) for i in range(num_blocks[0])])
        ...
        self.decoder_level2_0 = nn.Sequential(*[VSSM(dim=int(int(dim * 2 * 2))) for i in range(num_blocks[1])])
        ...
        self.decoder_level3_0 = nn.Sequential(*[VSSM(dim=int(int(dim * 2 * 4))) for i in range(num_blocks[2])])
        ...
        self.decoder_level4 = nn.Sequential(*[VSSM(dim=int(int(dim * 2 * 8))) for i in range(num_blocks[3])])
        ...
        self.decoder_level3_1 = nn.Sequential(*[VSSM(dim=int(int(dim * 2 * 4))) for i in range(num_blocks[2])])
        ...
        self.decoder_level2_1 = nn.Sequential(*[VSSM(dim=int(int(dim * 2 * 2))) for i in range(num_blocks[1])])
        ...
        self.decoder_level1_1 = nn.Sequential(*[VSSM(dim=int(int(dim * 2))) for i in range(num_blocks[0])])
        ...
        #self.latent = nn.Sequential(*[VSSM(dim=int(dim)) for i in range(num_blocks[0])])
        self.refinement_1 = nn.Sequential(*[VSSM(dim=int(dim)) for i in range(num_refinement_blocks)])
        self.refinement_2 = nn.Sequential(*[VSSM(dim=int(dim)) for i in range(num_refinement_blocks)])
        self.refinement_3 = nn.Sequential(*[VSSM(dim=int(dim)) for i in range(num_refinement_blocks)])
        self.refinement_4 = nn.Sequential(*[VSSM(dim=int(dim)) for i in range(num_refinement_blocks)])
        self.refinement_5 = nn.Sequential(*[VSSM(dim=int(dim)) for i in range(num_refinement_blocks)])
        self.refinement_6 = nn.Sequential(*[VSSM(dim=int(dim)) for i in range(num_refinement_blocks)])

        self.layer_fussion_3 = LAM_Module_v2(in_dim=int(dim*3))
        self.conv_fuss_3 = nn.Conv2d(int(dim * 3), int(dim), kernel_size=1, bias=bias)
        
        self.layer_fussion_4 = LAM_Module_v2(in_dim=int(dim*3))
        self.conv_fuss_4 = nn.Conv2d(int(dim * 3), int(dim), kernel_size=1, bias=bias)
    ...
    def forward(self, inp_img):
        Permute_layer1 = Permute(0,1,4,2,3)#before layer fusion
        Permute_layer2 = Permute(0,2,3,1)#channel to last before VSSM
        Permute_layer3 = Permute(0,3,1,2)#channel to first before sampling

        inp_enc_encoder1 = self.patch_embed1(inp_img)
        #print("tensor of out:", inp_enc_encoder1.shape)
        ...
        inp_enc_encoder2 = self.patch_embed2(inp_img)
        out_enc_encoder4 = self.encoder_4(inp_enc_encoder2)
        out_enc_encoder5 = self.encoder_5(out_enc_encoder4)
        out_enc_encoder6 = self.encoder_6(out_enc_encoder5)

        inp_fusion_123 = Permute_layer1(torch.cat([out_enc_encoder1.unsqueeze(1),out_enc_encoder2.unsqueeze(1),out_enc_encoder3.unsqueeze(1)],dim=4))
        

        inp_fusion_456 = Permute_layer1(torch.cat([out_enc_encoder4.unsqueeze(1),out_enc_encoder5.unsqueeze(1),out_enc_encoder6.unsqueeze(1)],dim=4))

        out_fusion_123 = self.layer_fussion_1(inp_fusion_123)
        out_fusion_123 = self.conv_fuss_1(out_fusion_123)
        out_fusion_123 = self.resize1(out_fusion_123)
        

        out_fusion_456 = self.layer_fussion_2(inp_fusion_456)
        out_fusion_456 = self.conv_fuss_2(out_fusion_456)
        #out_fusion_456 = self.resize2(out_fusion_456)

        out_fusion_1_6 = out_fusion_123 + out_fusion_456

        inp_enc_level1_0 = Permute_layer2(self.down_1(out_fusion_1_6))
        
        out_enc_level1_0 = Permute_layer3(self.decoder_level1_0(inp_enc_level1_0))



        inp_enc_level2_0 = Permute_layer2(self.down_2(out_enc_level1_0))
        out_enc_level2_0 = Permute_layer3(self.decoder_level2_0(inp_enc_level2_0))



        inp_enc_level3_0 = Permute_layer2(self.down_3(out_enc_level2_0))
        out_enc_level3_0 = Permute_layer3(self.decoder_level3_0(inp_enc_level3_0))



        inp_enc_level4_0 = Permute_layer2(self.down_4(out_enc_level3_0))
        out_enc_level4_0 = Permute_layer3(self.decoder_level4(inp_enc_level4_0))
        ...
        inp_enc_level3_1 = Permute_layer2(self.skip_4_3(inp_enc_level3_1))  ### conv 1x1


        out_enc_level3_1 = Permute_layer3(self.decoder_level3_1(inp_enc_level3_1))
        ...
        inp_enc_level2_1 = Permute_layer2(self.skip_3_2(inp_enc_level2_1))  ### conv 1x1


        out_enc_level2_1 = Permute_layer3(self.decoder_level2_1(inp_enc_level2_1))
        ...
        inp_enc_level1_1 = Permute_layer2(self.skip_1_0(inp_enc_level1_1))  ### conv 1x1

        out_enc_level1_1 = Permute_layer3(self.decoder_level1_1(inp_enc_level1_1))

        out_enc_level1_1 = self.up2_0(out_enc_level1_1)


        out_fusion_1_6 = self.latent(Permute_layer2(out_fusion_1_6))


        out = self.coefficient_1_0[0, :][None, :, None, None] * Permute_layer3(out_fusion_1_6)  + self.coefficient_1_0[1, :][None, :, None, None] *  out_enc_level1_1
        
        '''
        out_1 = self.refinement_1(Permute_layer2(out))
        out_2 = self.refinement_2(out_1)
        out_3 = self.refinement_3(out_2)
        inp_fusion = Permute_layer1(torch.cat([out_1.unsqueeze(1),out_2.unsqueeze(1),out_3.unsqueeze(1)],dim=1))
        out_fusion_123 = self.layer_fussion_2(inp_fusion)
        out = self.conv_fuss_2(out_fusion_123)
        '''

        
        out1 = self.patch_embed3(out)#8x8
        out2 = self.patch_embed4(out)#32x32
        
        out_1 = self.refinement_1(out1)
        out_2 = self.refinement_2(out_1)
        out_3 = self.refinement_3(out_2)
        inp_fusion_123 = Permute_layer1(torch.cat([out_1.unsqueeze(1),out_2.unsqueeze(1),out_3.unsqueeze(1)],dim=4))
        out_fusion_123 = self.layer_fussion_3(inp_fusion_123)
        out123 = self.conv_fuss_3(out_fusion_123)
        out123 = self.resize3(out123)

        out_4 = self.refinement_4(out2)
        out_5 = self.refinement_5(out_4)
        out_6 = self.refinement_6(out_5)
        inp_fusion_456 = Permute_layer1(torch.cat([out_4.unsqueeze(1),out_5.unsqueeze(1),out_6.unsqueeze(1)],dim=4))
        out_fusion_456 = self.layer_fussion_4(inp_fusion_456)
        out456 = self.conv_fuss_4(out_fusion_456)
        out456 = self.resize4(out456)
