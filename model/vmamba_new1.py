...
try:
    from .csm_triton_new import CrossScanTriton, CrossMergeTriton, CrossScanTriton1b1
except:
    from model.csm_triton_new import CrossScanTriton, CrossMergeTriton, CrossScanTriton1b1

# pytorch cross scan =============
#自定义自动求导函数 继承自autograd.Function类
...
class CrossScan_Ab_2direction(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor):
        B, C, H, W = x.shape
        ctx.shape = (B, C, H, W)
        x = x.view(B, 1, C, H * W).repeat(1, 2, 1, 1)#复制为B 2 C H*W
        x = torch.cat([x, x.flip(dims=[-1])], dim=1)#拼接出B 4 C H*W
        return x
    ...
    def backward(ctx, ys: torch.Tensor):
        B, C, H, W = ctx.shape
        L = H * W
        ys = ys[:, 0:2] + ys[:, 2:4].flip(dims=[-1]).view(B, 2, -1, L)#应该是B 2 C H*W
        return ys.sum(1).view(B, -1, H, W)#先是在第二个维度相加得到B C H*W
...
# fvcore flops =======================================
#计算某个selective scan的浮点操作数 FLOPs
...
#更精确的方法来估算selective scan的FLOPs
#当with_Group=True时 函数考虑了两个不同的分组
#这可能导致更多的FLOPs 因为需要计算两个分组的相互作用
...
#打印给定输入张量或参数的调试名称
#比如输入三个张量 打印出tensor1 tensor2 tensor3作为调试名称
...
class SelectiveScanMamba(torch.autograd.Function):
    ...
    #amp用于自动化混合精度训练 FP16 FP32
    #这个装饰器允许用户自定义前向传播的操作
    #使用amp自动执行必要的类型转换和计算优化
    ...
    #dout是对输出的梯度 损失函数对于输出的梯度
    def backward(ctx, dout, *args):
        u, delta, A, B, C, D, delta_bias, x = ctx.saved_tensors
        if dout.stride(-1) != 1:#检查在最后一个维度上的步长 判断是否连续
        ...
        #None表示没有被计算或者不需要返回这些值


#使用了selective_scan_cuda_core
...
class PatchMerging2D(nn.Module):
    def __init__(self, dim, out_dim=-1, norm_layer=nn.LayerNorm, channel_first=False):
        #norm_layer：用于归一化的层或函数 默认为nn.LayerNorm
        super().__init__()
        self.dim = dim
...
class SS2D(nn.Module):
    ...
    def __initv0__(
        ...
        **kwargs,
        ...
        #检查kwargs中是否存在key channel_first 
        #若存在 验证其对应的值是否为false
        ...
        self.forward = self.forwardv0 #989行
        #partial作用为预先设置forwardv0中的一些参数
        if seq:
        ...
        #在第一个维度上把weights堆叠起来 4 d_inner dt_rank + d_state * 2
    ...
    def __initv2__(
        ...
        **kwargs,    
        ...
        def checkpostfix(tag, value):
            ...
            if ret:
                value = value[:-len(tag)]#如果value以tag结尾 则从value中移除该tag
            return ret, value
        ...
        if forward_type[-len("none"):] == "none":
            ...
            self.out_norm = nn.Identity()#设置一个恒等映射 不对输入做任何修改
            ...
            class SoftmaxSpatial(nn.Softmax):
                def forward(self, x: torch.Tensor):
                    B, C, H, W = x.shape
                    #调用父类nn.Softmax的forward方法
                    return super().forward(x.view(B, C, -1)).view(B, C, H, W)
            self.out_norm = SoftmaxSpatial(dim=-1)#在最后一个维度上进行
        ...
        # forward_type debug =======================================
        #每个键值对对应一个前向传播方法
        #partial的作用为预先设置foward_corev2中的一些特定参数
        #1068行
        FORWARD_TYPES = dict(
        ...
        self.forward_core_real = FORWARD_TYPES.get(forward_type, None)
        self.forward_core_imag = FORWARD_TYPES.get(forward_type, None)
        #如果给定的forward_type不存在于dict中 forward_core设置为None
        k_group = 4
        ...
        self.act: nn.Module = act_layer()
        #self.act将是一个act_layer函数返回的nn.Module对象的实例
    ...
    def dt_init(dt_rank, d_inner, dt_scale=1.0, dt_init="random", dt_min=0.001, dt_max=0.1, dt_init_floor=1e-4, **factory_kwargs):
        ...
        if dt_init == "constant":
            nn.init.constant_(dt_proj.weight, dt_init_std)#权重初始化为常数dt_init_std
        elif dt_init == "random":
            nn.init.uniform_(dt_proj.weight, -dt_init_std, dt_init_std)#权重初始化为范围内均匀分布的随机值
        else:
        ...
        with torch.no_grad():
            dt_proj.bias.copy_(inv_dt)#将inv_dt复制给bias
    ...
    def A_log_init(d_state, d_inner, copies=-1, device=None, merge=True):
        ...
        if copies > 0:
            ...
            if merge:
                A_log = A_log.flatten(0, 1)#将第0和第1维度扁平化
        A_log = nn.Parameter(A_log)
        A_log._no_weight_decay = True#不受权重衰减的影响
        return A_log
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        delta_bias = dt_projs_bias.view(-1).to(torch.float)#转换为一维向量

        #在forward_type = v2时 force_fp32=(not self.disable_force32)
        if force_fp32:
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        #nn.Linear
        x = self.in_proj(x)
        #disable_z = False, disable_z_act = False
        ...
        if not self.channel_first:
            x = x.permute(0, 3, 1, 2).contiguous()
            x_fft = fft.fft2(x, dim=(2,3))
            x_fft_real = x_fft.real
            x_fft_imag = x_fft.imag
        if with_dconv:
            x = self.conv2d(x) # (b, d, h, w)
            x_fft_real = self.conv2d_real(x_fft_real)
            x_fft_imag = self.conv2d_imag(x_fft_imag)
        x = self.act(x)
        x_fft_real = self.act(x_fft_real)
        x_fft_imag = self.act(x_fft_imag)
        
        x = self.forward_core(x)
        x_fft_real = self.forward_core_real(x_fft_real)
        x_fft_imag = self.forward_core_imag(x_fft_imag)
        #v2=partial(self.forward_corev2, force_fp32=(not self.disable_force32), SelectiveScan=SelectiveScanCore)

        x_fft = torch.complex(x_fft_real, x_fft_imag)
        y = x + fft.ifft2(x_fft, dim=(2,3)).real
...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        # 归一化层
        self.norm_qk = norm_layer(self.head_dim)
        self.norm_v = norm_layer(self.head_dim)
        
        # 输出的线性变换层
        self.fc_o = nn.Linear(embed_size, embed_size)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 线性变换得到 Q, K, V
        q = self.fc_q(x).reshape(B, L, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        k = self.fc_k(x).reshape(B, L, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        v = self.fc_v(x).reshape(B, L, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        ...
        # 合并多头注意力输出
        attn_out = attn_out.permute(0, 2, 1, 3).contiguous().reshape(B, L, self.embed_size)
        
        # 线性变换得到输出
        attn_out = self.fc_o(attn_out)
        attn_out = attn_out.reshape(B, H, W, D)
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        ...
        B, H, W, D = input.shape
        L = H * W
        value = input.reshape(B, L, D)
        query = input.reshape(B, L, D)
        key = input.reshape(B, L, D)
        attn_output, attn_output_weights = self.attention(query, key, value)
        attn_output = attn_output.reshape(B, H, W, D)
        '''
        if self.ssm_branch:
            if self.post_norm:
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        #把norm_layer变为小写 检查是否为bn或者ln2d
        self.channel_first = (norm_layer.lower() in ["bn", "ln2d"])
        #self.num_classes = num_classes
        #4层layers
        self.num_layers = len(depths)
        #检查dims是否为一个整数
        #2的i_layer次方 从0到num_layers-1
        #if isinstance(dims, int):
            #dims = [int(dims * 2 ** i_layer) for i_layer in range(self.num_layers)]
        #self.num_features = dims[-1]
        #self.dims = dims
        self.dim = dim
        #生成一个均匀间隔的序列 从0到drop_path_rate 长度为sum
        ...
        #将输入图像分解为固定大小的patch patch embedding
        ...
        _make_downsample = dict(
            ...
            #匿名函数 接受任意数量的位置参数和关键字参数 返回值始终是None
        ).get(downsample_version, None)

        #定义一个空的list 用于储存网络层或者模块
        ...
        for i_layer in range(self.num_layers):
            ...
            #生成layer 并添加入self.layers中
            self.layers.append(self._make_layer(
                dim = self.dim,
                #返回dpr序列中的一段 从0开始 左闭右开
        ...
        self.classifier = nn.Sequential(OrderedDict(
            ...
            avgpool=nn.AdaptiveAvgPool2d(1),# B C 1 1
            flatten=nn.Flatten(1),# B -1
