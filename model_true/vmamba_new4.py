...
class IFM(nn.Module):
    def __init__(self, n_feat):
        ...
        self.conv = nn.Conv2d(n_feat, n_feat, kernel_size=1, stride=1, bias=False)

    def forward(self, input1: torch.Tensor, input2: torch.Tensor):
        x = input1 * input2
        output = x * self.conv(x.permute(0,3,1,2)).permute(0,2,3,1)
        return output
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input1: torch.Tensor, input2: torch.Tensor = None):
        if self.ssm_branch:
            if self.post_norm:
                x = input1 + self.drop_path(self.norm(self.op(input1)))
            else:
                x = input1 + self.drop_path(self.op(self.norm(input1)))
        ...
        if input2 is not None:
            x = x + self.drop_path(self.ifm(self.norm2(x), input2))
